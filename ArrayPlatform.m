classdef ArrayPlatform < matlab.System
    ...
    methods
        ...
        function computeNormMatrix(obj)
            ...
            %    SvNoNorm = ...
            SvNoNorm = db2mag(elemGainVal').*Sv0;
            
            % TODO:  Compute the normalization matrix by integrating the
            % un-normalized steering vectors
            %    Q = ...           
            nelVal = length(elVal);
            coselVal = cos(deg2rad(elVal));
            Q = (1/nelVal/mean(coselVal))*(SvNoNorm.*coselVal'*SvNoNorm');
    ...
    methods (Access = protected)
       ...
       function [Sv, elemGain] = stepImpl(obj, az, el, relSV)
            ...
            localcoords = global2localcoord( ...
                [az el ones(length(el), 1)]', ...
                'ss', ...
                zeros(3, 1), ...
                obj.axesLoc);
            azLocal = localcoords(1,:);
            elLocal = localcoords(2,:);

            % TODO: Get the SV in the local coordinates
            %    Sv0 = obj.svObj(...)
            Sv0 = obj.svObj(obj.fc, [azLocal; elLocal]);
                        
            % TODO:  Get the directivity gain of the element from the
            % local angles.
            %    elemGain = obj.elem.step(...) 
            elemGain = obj.elemGainInterp(elLocal,azLocal)';
