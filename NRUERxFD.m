classdef NRUERxFD < matlab.System
    ...
    methods
        ...
        function chanEst(obj, rxGrid)
            ...
            NSc = size(rxGrid, 1);
            dmrsSymNum = double(floorDiv(dmrsInd-1, NSc) + 1);
            ...
            for i = 1:ndrmsSym
                ...
                tmp_sym = dmrsSymNums(i);
                I = find(dmrsSymNum == tmp_sym);
                ...
                chanEstDmrs(:,i) = kernelReg(ind,raw,NSc,obj.lenFreq,...
                    obj.sigFreq);
    ...
    methods (Access = protected)
        ...
        function rxBits = stepImpl(obj, rxGrid, chanGrid, noiseVar)
            ...
            nom = conj(obj.pdschChan).*obj.pdschSym;
            denom = abs(obj.pdschChan).^2 + obj.noiseEst;

            obj.pdschSymEq = nom ./ denom;
