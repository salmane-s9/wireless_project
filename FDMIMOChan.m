classdef FDMIMOChan < matlab.System
    ...
    methods (Access = protected)
        ...
        function [chanGrid, noiseVar] = stepImpl(obj, frameNum, slotNum)
            ...
            for i = 1:npath
                phase = reshape(2*pi*(f*obj.dly(i) + t'*obj.fd(i)),1,1,nsc,nsym);
                svRx_ = reshape(obj.svRx(:,i), nrx,1,1,1);
                svTx_ = reshape(obj.svTx(:,i), 1,ntx,1,1);
                chanGrid_i = svRx_.*svTx_ * obj.gainComplex(i).*exp(1i*phase);
                chanGrid = chanGrid + chanGrid_i;
            end
